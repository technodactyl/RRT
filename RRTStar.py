...
class Node:
  def __init__(self, v: Vector2):
    ...
    self.path_length = 0
    self.neighborhood_radius = 1.2 * unit
    self.neighbors = []
  ...
  def add_neighbor_nodes(self, nodes):
    for node in nodes:
      if (self.pos - node.pos).length() < self.neighborhood_radius and len(self.neighbors) < max_neighbors:
        self.neighbors.append(node)
   
  def check_optimal_neighbors(self):
    min_dist = 5000
    for neighbor in self.neighbors:
      dist = neighbor.path_length + (self.pos - neighbor.pos).length()
      if dist < min_dist:
        min_dist = dist
        optimal_parent = neighbor
    self.add_parent_node(optimal_parent)
    
    self.neighbors.remove(self.parent_node)
    for neighbor in self.neighbors:
      dist = self.path_length + (self.pos - neighbor.pos).length()
      if dist < neighbor.path_length:
        neighbor.add_parent_node(self) 
      

  def step_to_point(self, point: Vector2, nodes):
    unit_step = (point - self.pos)/(point - self.pos).magnitude() * unit
    new_node = Node(self.pos + unit_step)
    new_node.add_neighbor_nodes(nodes)
    new_node.check_optimal_neighbors()
    return new_node
...
if __name__ == '__main__':
  ...
  while running:
    ...
    while len(nodes) < max_points:
      ...
      new_node = root_node.step_to_point(random_point, nodes)
      for obstacle in obstacles:
